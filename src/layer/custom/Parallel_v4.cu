...
__global__ void conv_forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    int startOfTile_h = (blockIdx.y / w_grid) * TILE_WIDTH; // row of the input image matrix
    int startOfTile_w = (blockIdx.y % w_grid) * TILE_WIDTH; // col of the input image matrix
        ...
        for(int i = threadIdx.y; i < INPUT_TILE_WIDTH; i += TILE_WIDTH)
        {
            ...
            for(int j = threadIdx.x; j < INPUT_TILE_WIDTH; j += TILE_WIDTH)
            {
                ...
                {
                    smem(d, i, j) = x4d(b, d, startOfTile_h + i, startOfTile_w + j);
                }  
            }
        }
